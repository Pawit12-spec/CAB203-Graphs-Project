...
def minColouring(V,E, k=None, C=None, kbest=None, Cbest=None, Vrem=None):
   ...
   if k is None:
      ...
      k = 0                       # no colours used yet
      Vrem = list(V)              # vertices to colour remaining
   
   # If no more vertices to colour then we are done.   
   if not Vrem:
      if k < kbest:
         # Copy C so that Cbest doesn't get changed by the caller
         return k, dict(C)
      return kbest, Cbest

   # Grab a vertex to colour
   v = Vrem.pop(-1)
   badcolours = { C[u] for u in N(V, E, v) } - { None }
   
   if len(badcolours) + 1 >= kbest:
      # We will use at least kbest colours.  Not an improvement, so return
      Vrem.append(v)
      return kbest, Cbest

   # Try all possible colours, including adding a new colour
   for c in range(k + 1):
      if c in badcolours: continue     # a neighbour has this colour, skip.
      C[v] = c                         # Try to assign this colour       
      nextK = k + 1 if c == k else k   # Number of colours used so far

      # Find the best colouring that extends C
      kret, Cret = minColouring(V, E, nextK, C, kbest, Cbest, Vrem)  
         
      # If we found a better colouring, keep it.
      if kret < kbest:
         kbest, Cbest = kret, Cret

   # Undo what we did as C and VRem are shared with the caller
   C[v] = None
   Vrem.append(v)
